...
[![Tests Passing](https://img.shields.io/badge/Unit%20Tests-152%20Passed-brightgreen.svg)](#automated-testing--quality)
[![Tools Count](https://img.shields.io/badge/Utilities-61%20Tools-blue.svg)](#utility-catalog-61-tools)
...
- **61 Production-Grade Utilities**: From everyday math and unit conversions to secure cryptographic operations, file forensics, and document builders.
...
- **Visual Tool Chaining ("Send To")**: Pass calculation results and text outputs directly between compatible downstream tools seamlessly.
- **High-Performance In-Memory Search Engine**: Sub-millisecond fuzzy search with synonym dictionaries and alias expansion.
- **152 Automated Unit Tests**: 100% test pass rate covering algorithms, edge cases, cryptographic vectors, and exploit defenses.

---

## Utility Catalog (61 Tools)

### 1. File Utilities & Forensics (3 Tools)
...
### 2. Image & Media Forensics (2 Tools)
...
### 3. Math, Finance & Everyday Calculators (8 Tools)
...
com.offline.toolbox
├── core
│   ├── database             # Room DB: favorites, recent usage, persisted offline state
│   ├── datastore            # DataStore: user preferences, theme, dynamic color
│   ├── designsystem         # Material 3 design tokens, components, ToolScaffold
│   └── model                # Core Tool contract, capabilities, types, results
├── engine
│   ├── registry             # Central ToolRegistry indexing all 61 utilities
│   └── search               # In-memory fuzzy search engine with synonym dictionary
├── features
│   ├── home                 # Category grid, favorites, recents, search bar
│   ├── settings             # Offline privacy audit, theme picker, data wipe
│   └── toolrunner           # Dynamic tool execution screen, tool chaining dialog
└── tools                    # Decoupled, modular tool implementations by domain
    ├── color
    ├── converter
    ├── data
    ├── datetime
...
### Extensible Tool Contract
Every utility implements the unified `Tool<Input, Output>` contract:
```kotlin
...
The application manifest (`app/src/main/AndroidManifest.xml`) declares **ZERO** network permissions:
```xml
<!-- NO android.permission.INTERNET -->
<!-- NO ACCESS_NETWORK_STATE -->
<!-- Pure offline local calculation -->
```
...
All 61 utilities are backed by comprehensive automated unit test suites covering edge cases, boundary values, security sanitization, and cryptographic outputs:
